...
| "AR 195-5 permits (or prescribes) the year-first layout `26-01`." | It does neither. **2-4c** describes exactly one layout: a three-digit sequence beginning at 001, a hyphen, then the two-digit calendar year (`001-18`). A room that writes the year first is following a **local** practice; EMC records such numbers as written under a per-room policy that must cite a local authority or be flagged as awaiting validation (VCH-023), and the canonical identity `(room, year, sequence)` is what EMC compares. EMC must never describe the local layout as regulatory. |
| "Para 2-5b(5) (one line through the entry, initialed) is the DA Form 4137 correction procedure." | **2-5b(5) governs erroneous entries in the EVIDENCE LEDGER.** The DA Form 4137 review procedure is **2-3g**: the custodian has the submitting agent "correct and initial all errors". EMC models a 2-3g correction as a form revision (VCH-025) with the agent's attestation that the paper form was corrected and initialed; it imports neither the ledger strike-through rule nor a 1-7c(3) MFR into that workflow. |
