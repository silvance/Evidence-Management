...
| Temporary release: one unit of work, custody and status events per item, paper original out and first copy filed, attestations (never signatures), accountable mail for USACIL only, one open release per item at the database, append-only contacts, room-scoped reads | D `TemporaryReleaseTests.*`; A `TemporaryReleaseServiceTests.*` |
