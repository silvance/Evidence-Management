...
| Temporary release and return: one unit of work each way, custody and status events per item, paper original out and back with the first copy, copies and multi-recipient releases, attestations (never signatures), accountable mail for USACIL only, laboratory coordination and the DFT copy, 2-7d apparent change with MFR, not-returned accounting, one open release per item at the database, append-only contacts, room-scoped reads | D `TemporaryReleaseTests.*`, `PhysicalDocumentTests.*`; A `TemporaryReleaseServiceTests.*` |
...
| Custody backfill from a scan: only by the custodian's explicit act, historical OccurredAt, scan as provenance, finding consumed once, no status change, no release, no 1-7c(3) MFR | A `ReconciliationCustodyTests.*` |
...
| Suspense dashboard: the regulation's three folders, PENDING DISPOSITION APPROVAL from the paper record, days out as a count, the one threshold LOCAL and never a deadline, follow-ups the custodian set, room scoped | A `SuspenseDashboardTests.RowsSitUnderTheRegulationsFolders_DaysOutIsACount_AndTheOnlyThresholdIsLocal`, `SuspenseDashboardTests.AContactResetsTheLastContactColumn_AndAClosedReleaseMovesToRecentlyClosed`, `SuspenseDashboardTests.PendingDispositionApprovalRowsComeFromThePaperRecord_NotFromAReleaseOfEvidence`, `SuspenseDashboardTests.TheDashboardIsARoomScopedRead` |
| Release/companion consistency advisories (SCV-001..008) change nothing and reach the voucher consistency report | A `SuspenseDashboardTests.AdvisoriesNameEachDisagreement_ChangeNothing_AndReachTheVoucherConsistencyReport` |
| Temporary release, contact, dashboard and return through the real pages with anti-forgery; the custody form on the item page with the reconciliation hand-off prefilled; outsider gets 404 on the release page and the dashboard | A `SuspenseHttpTests.ReleaseContactDashboardAndReturnThroughThePages_ThenNothingForAnOutsider`, `SuspenseHttpTests.TheItemHistoryPageOffersTheCustodyForm_AndAnUnknownReleaseIs404` |
...
| SQL Server lane without a SQL Server: every column the lane inserts or updates exists in the schema script and no NOT NULL column is omitted (error 207/515 found by review); every trigger in the idempotent DBA script runs in its own batch through EXEC (error 111) with distinct THROW numbers | A `OfflineBuildTests.TheSqlServerLaneNamesOnlyColumnsTheSchemaHas`, `OfflineBuildTests.EveryTriggerInTheSchemaScriptIsItsOwnBatch` |
| Release bundle: runtimes pinned solution-wide and in every lock file, never self-contained; the export carries the apphost packs; the producers restore locked and offline, publish framework-dependent with the commit stamped, strip development files, and mark a dry run, a dirty tree or a skipped test run; the verifiers cross-check both manifests, the entry points, the IIS handler and the absence of credentials; no script names a URL | A `OfflineBuildTests.TheReleaseBundleIsNotCommitted`, `OfflineBuildTests.TheRuntimesArePinnedSolutionWide_NeverSelfContained_AndEveryLockFileCarriesThem`, `OfflineBuildTests.TheDependencyBundleExportCarriesTheApphostPacks`, `OfflineBuildTests.TheReleaseScriptsReachNoNetwork_RestoreLocked_AndAgreeOnTheBundle` |
