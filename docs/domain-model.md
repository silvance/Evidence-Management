...
| `PhysicalVoucherDocument` / `PhysicalVoucherDocumentEvent` | **✚ ADDED. [REG] 2-4d, 2-4f, 2-4g, 2-4h, 2-7g.** Where the PHYSICAL original DA Form 4137 is for one voucher — filed active, out with released evidence (suspense copy retained), sent for disposition approval, filed inactive, transferred to the gaining room, or copy-only because the original is unavailable — plus the inactive date, the computed 3-year eligibility, and confirmed destruction. Voucher-level; independent of any scan; knows nothing of the case. Events append-only. |
