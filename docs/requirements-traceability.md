...
| VCH-010 | Items may be added, edited or removed only while the voucher is a draft or has been returned by the custodian for correction | 2-3g | REG | Implemented | `EvidenceItemTests.ItemsCannotBeEditedOnceTheVoucherIsSubmitted`, `VerticalSliceTests.ItemsCannotBeAddedOnceTheVoucherHasBeenSubmitted`, `VoucherReviewTests.AReturnedVoucherIsEditableAgain` |
...
| VCH-021 | An item that already carries accountability events cannot be removed from a returned voucher; it is corrected instead | 2-5b(5) modelled | CONTROL | Implemented | `CustodianReviewTests.AnItemAlreadyOnTheRecordCannotBeRemovedFromAReturnedVoucher`, `CustodianReviewTests.AnItemAddedDuringCorrectionJoinsTheResubmission` |
...
| LOC-008 | Whether an item may hold an evidence-room location is decided by a named predicate on the state machine, not by enum order or a hand-written list; an item in a temporary facility (4-3a) cannot | 2-4e, 4-3a | REG | Implemented | `AccountabilityStateTests.PreAcceptanceStatesCannotHoldAnEvidenceRoomLocation`, `AccountabilityStateTests.ReceivedItemsMayHoldALocationUnlessTerminal` |
