...
| FIL-010 | A room dashboard of the paper DA Form 4137 files: Active, Suspense, Inactive split into Retain / Eligible for destruction / Destruction confirmed by the 3-year clock from the inactive date alone; accepted vouchers with no filing recorded; folder load against the 50-voucher limit | 2-4f(1), 2-4f(3), 2-4h | REG (rule) / DESIGN (dashboard) | Implemented | `PaperRecordReportTests.TheDashboardBucketsByFileAndByTheThreeYearClock_FromTheInactiveDateOnly`, `PaperDashboardHttpTests.TheRetentionDashboardRendersForTheRoom_AndNotForAnOutsider` |
| PDC-001 | Physical/digital cross-checks are ADVISORIES only: the paper record (where the original and its copies are) against the companion record (item states, companion copies); nothing is changed by a check, and the paper remains authoritative (2-5c) | 2-4d, 2-4f(2), 2-4g, 2-4h, 2-7a, 2-7g | DESIGN | Implemented | `PaperRecordReportTests.AdvisoriesSayWhatDisagrees_AndChangeNothing` — PDC-001 accepted but unfiled; PDC-002/003 temporary release vs original location; PDC-004/005 inactive file vs item states; PDC-006 transfer; PDC-007 copy without reason; PDC-008 destruction-eligible; PDC-020..022 companion-copy notes |
...
| DOC-007 | MFRs are attached to the appropriate DA Form 4137 and copied to the case file | 1-7c(3), 2-3e, 3-2f, 3-3a | REG | Partial | A companion copy of an MFR is received as `SourceDocumentType.MemorandumForRecord` against the voucher (`SourceDocumentTests.AValidPdfIsStoredHashedAndRendered` covers the path); the attachment to the PAPER form and the copy in the case file are paper acts recorded by the custodian, not performed by software |
| DOC-008 | Receipts and chain-of-custody documents from non-Army agencies are attached to the DA Form 4137 | 2-3g | REG | Partial | Received as `SourceDocumentType.ExternalChainOfCustody`; OCR falls back to generic lines for such a page (`DaForm4137Face.Unknown`), and its custody rows are surfaced at reconciliation (REC-003). The paper attachment is the custodian's act |
...
| SUSP-007 | On temporary release the original DA Form 4137 accompanies the evidence and a copy is retained in the suspense folder | 2-4f(2), 2-7b | REG | Implemented (paper states) | `PhysicalDocumentTests.TemporaryReleaseSendsTheOriginalAndKeepsASuspenseCopy`, `PhysicalDocumentTests.TheOriginalReturnsToTheActiveFile` |
...
| RET-007 | Inactive DA Forms 4137 are disposed of 3 years after becoming inactive; ledgers may be disposed 3 years after the last item is disposed | 2-4h, 2-5a | REG | Implemented (paper retention clock and dashboard) | `PhysicalDocumentTests.InactiveFilingStartsTheThreeYearClock_ExactlyThreeYears`, `PhysicalDocumentTests.EligibleIsNotDestroyed_AndDestructionIsConfirmedByAPerson` |
